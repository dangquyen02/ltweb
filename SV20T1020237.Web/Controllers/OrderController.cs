...
namespace SV20T1020237.Web.Controllers
{
    ...
    public class OrderController : Controller
    {
            ...
            if (input == null)
            {
                input = new OrderSearchInput()
                {
                    ...
                    DateRange = string.Format("{0:dd/MM/yyyy} - {1:dd/MM/yyyy}",
                                                DateTime.Today.AddMonths(-40),   //"02/01/2021",//DateTime.Today.AddMonths(-1)
                                                DateTime.Today)
                };
            }
            return View(input);
        ...
        [HttpPost]
        public IActionResult EditAddressOrder(int id = 0, string deliveryProvince = "", string deliveryAddress = "")
        {
            if (deliveryAddress == null || deliveryProvince == null)
            {
                return Json("Điền đầy dủ thông tin địa chỉ");
            }
            bool result = OrderDataService.SaveAddressOrder(id, deliveryProvince, deliveryAddress);
            if (!result)
            {
